...
import { Project, Tag, PartialDocument, DocumentSearchResult } from '~/lib/types'
...
const makeListSource = <T,>({
  ...
  (searchQuery, handleAction) => list.map(
    item => makeDynamicSuggestion(item, handleAction, rest)
  )
);

interface MakeFilteredListSourceOptions<T> extends MakeDynamicSuggestionOptions<T> {
  list: T[];
...
interface MakeSingletonSourceOptions extends Omit<Suggestion, 'key' | 'label' | 'onCommit'> {
  name: string;
  action: () => void;
}
...
const makeSingletonSource = ({
  ...
  action,
  ...rest
}: MakeSingletonSourceOptions): SuggestionSource => (
  (searchQuery, handleAction) => includes(name, searchQuery)
    ? [{
      key: name,
      label: name,
  ...
  const suggestions: Suggestion[] = useMemo(() => {
    const suggestionSources: SuggestionSource[] = [
      ...
      makeSingletonSource({
        ...
        action: () => navigate(editProjectPath({ projectId: currentProject.id })),
      }),
      ...
      makeSingletonSource({
        ...
        action: () => navigate(recentlyViewedPath({ projectId: currentProject.id })),
      }),
      ...
      makeFilteredListSource({
        ...
        action: ({ id }) => navigate(tagPath({ projectId: currentProject.id, tagId: id })),
      ...
      makeFilteredListSource({
        ...
        action: ({ id }) => navigate(documentPath({ projectId: currentProject.id, documentId: id })),
      }),
      ...
      makeListSource({
        ...
        action: ({ document: { id } }) => navigate(documentPath({ projectId: currentProject.id, documentId: id })),
      }),
    ];
