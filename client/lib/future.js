const Future = {
  makeFuture: (type, data = undefined) => ({
    ...
    map: (f) =>
      type === 'resolved' ? Future.resolved(f(data)) : Future.pending(),
  }),
...
const ServiceResult = {
  makeServiceResult: (type, data = undefined) => ({
    ...
    map: (f) =>
      type === 'success'
        ? ServiceResult.success(f(data))
        : ServiceResult.failure(),
  }),

  fromPromise: (promise, consumer) =>
    promise.then(
...
const FutureServiceResult = {
  ...
  success: (data) =>
    FutureServiceResult.fromFuture(
      Future.resolved(ServiceResult.success(data))
    ),
  failure: (error) =>
    FutureServiceResult.fromFuture(
      Future.resolved(ServiceResult.failure(error))
    ),
};

// sequence :: ({ k: m a }, a -> m a) -> m { k: a }
const sequence = (collection, pure) =>
...
export { Future, ServiceResult, FutureServiceResult, sequence };
